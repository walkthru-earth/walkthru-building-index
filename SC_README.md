...
[Global Building Atlas LOD1](https://source.coop/tge-labs/globalbuildingatlas-lod1/) (2.75B buildings) aggregated to [H3](https://h3geo.org/) hexagonal cells with urban density metrics, in [native Parquet 2.11+ GEOMETRY](https://github.com/apache/parquet-format/blob/master/Geospatial.md) format. Six H3 resolutions (3–8), one file per resolution, sorted by `h3_index`.
...
FROM read_parquet('s3://us-west-2.opendata.source.coop/walkthru-earth/indices/building/h3/h3_res=5/data.parquet')
...
df = con.sql("""
    ...
    FROM read_parquet(
        's3://us-west-2.opendata.source.coop/walkthru-earth/indices/building/h3/h3_res=5/data.parquet'
...
**Total: 53,632,093 cells (~2.6 GB).** Compression: ZSTD level 3. Row groups: 1,000,000 rows.
...
**Sample values** (res 5, most built-up cells):

| h3_index | lat | lon | building_count | density/km² | avg_height_m | coverage |
|----------|-----|-----|----------------|-------------|--------------|----------|
| 85a81073fffffff | -23.5 | -46.5 | 998,869 | 3,904 | 5.6 | 0.343 |
| 856b6eaffffffff | +15.7 | +32.5 | 983,761 | 3,591 | 2.1 | 0.200 |
| 85424d07fffffff | +31.5 | +74.3 | 918,389 | 3,326 | 4.4 | 0.318 |
...
6. Final Parquet is sorted by `h3_index` with native GEOMETRY via DuckDB spatial
...
SELECT h3_index, lat, lon, max_height_m, avg_height_m, building_count
FROM read_parquet('s3://us-west-2.opendata.source.coop/walkthru-earth/indices/building/h3/h3_res=8/data.parquet')
...
SELECT b.h3_index, b.lat, b.lon,
       b.building_count,
       p.pop_2025,
       (b.building_count::FLOAT / NULLIF(p.pop_2025, 0))::DECIMAL(6,3) AS buildings_per_person
FROM read_parquet('s3://us-west-2.opendata.source.coop/walkthru-earth/indices/building/h3/h3_res=5/data.parquet') b
JOIN read_parquet('s3://us-west-2.opendata.source.coop/walkthru-earth/indices/population/scenario=SSP2/h3_res=5/data.parquet') p
...
FROM read_parquet(
    'https://data.source.coop/walkthru-earth/indices/building/h3/h3_res=5/data.parquet'
)
